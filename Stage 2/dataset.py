...
def download_mnist_dataset(data_path: str = "./data"):
    """
    ...
    transform = Compose([ToTensor(), Normalize((0.1307,), (0.3081,))]) # mean and std of MNIST dataset

    # Download MNIST datasets
    training_set = MNIST(data_path, train = True, download = True, transform = transform)
...
def prepare_federated_dataset(num_partitions: int, batch_size: int, val_ratio: float = 0.1):
    ...
    trainsets = random_split(
        trainset, partition_len, generator=torch.Generator().manual_seed(2275285)
    )
    ...
    for trainset in trainsets:
        ...
        for_train, for_val = random_split(
            trainset, [num_train, num_val], generator=torch.Generator().manual_seed(2275285)
        )
